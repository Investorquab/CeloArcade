...
export default function MathLobby() {
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      if (liveRoomCount !== undefined && liveRoomCount > 0n) {
        router.push(`/math/room/${liveRoomCount - 1n}`);
      }
    }
  ...
  function handleCreateRoom() {
    ...
    writeContract({
      ...
      args: [GAME_TYPE_MATH, 2, stakeWei, vsAI],
      value: stakeWei,
    });
  }
  ...
  return (
    <main className="min-h-screen px-5 py-6 max-w-md mx-auto">
      ...
      <h1 className="text-xl font-medium mb-1">Quick Math</h1>
      <p className="text-sm text-gray-500 mb-6">Fastest correct answer wins</p>
...
function RoomCard({ roomId }: { roomId: bigint }) {
  ...
  useEffect(() => {
    if (joinSuccess) router.push(`/math/room/${roomId}`);
  }, [joinSuccess, roomId, router]);

  if (!room) return null;
  const [host, gameType, maxPlayers, stakeAmount, vsAI] = room;
  if (gameType !== GAME_TYPE_MATH) return null;
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-3 flex justify-between items-center">
      ...
      <button
        onClick={isHost ? () => router.push(`/math/room/${roomId}`) : handleJoin}
